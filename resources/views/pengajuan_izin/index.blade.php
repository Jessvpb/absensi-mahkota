            ...
            @php
                ...
                $menunggu = $dataPengajuan
                    ->filter(function ($item) {
                        return is_null($item->validasi_admin) && $item->validasi_kepalacabang !== 0;
                    })
                    ->count();
                    ...
                    <thead class="bg-gray-800/50 border-b border-gray-700/50">
                        <tr>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">
                                <i class="fas fa-user mr-2 text-yellow-400"></i> Staff
                            </th>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">
                                <i class="fas fa-calendar mr-2 text-yellow-400"></i> Tanggal Pengajuan
                            </th>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">
                                <i class="fas fa-calendar-days mr-2 text-yellow-400"></i> Jumlah Hari
                            </th>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">
                                <i class="fas fa-tasks mr-2 text-yellow-400"></i> Status
                            </th>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">
                                <i class="fas fa-user-shield mr-2 text-yellow-400"></i> Admin Validasi
                            </th>
                            <th class="px-6 py-4 text-center text-sm font-semibold text-gray-300 uppercase tracking-wider">
                                <i class="fas fa-cogs mr-2 text-yellow-400"></i> Aksi
                            </th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-800/30 transition-colors duration-200">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-gray-400 text-sm">
                                                {{ $izin->staff->jabatan->first()->nama_jabatan ?? 'Jabatan tidak tersedia' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <span
                                            class="text-purple-400 font-semibold">{{ $izin->detail_pengajuan_izin->count() }}
                                            Hari</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    @if ($izin->validasi_kepalacabang === null)
                                        <span
                                            ...
                                            <i class="fas fa-clock mr-1"></i> Menunggu Kacab
                                        </span>
                                    @elseif ($izin->validasi_kepalacabang === 1 && $izin->validasi_admin === null)
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-500/20 text-blue-400 border border-blue-500/30">
                                            <i class="fas fa-user-shield mr-1"></i> Menunggu Admin
                                        </span>
                                    @elseif ($izin->validasi_admin === 1 && $izin->validasi_kepalacabang === 1)
                                        <span
        ...
        @if ($dataPengajuan->count() > 0)
            <div class="glass-card rounded-2xl p-6">
                <h3 class="text-xl font-semibold text-white mb-6 flex items-center">
                    <i class="fas fa-history mr-2 text-yellow-400"></i> Aktivitas Terbaru
                </h3>
                <div class="space-y-4">
                    @foreach ($dataPengajuan->take(5) as $recent)
                        <div class="flex items-center p-4 bg-gray-800/30 rounded-lg border border-gray-700/50">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-gray-400 text-sm">{{ $recent->detail_pengajuan_izin->count() }} hari •
                                    {{ $recent->created_at->diffForHumans() }}</p>
                            </div>
                            <div>
                                @if ($recent->validasi_kepalacabang === null)
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-medium bg-yellow-500/20 text-yellow-400">Menunggu
                                        Kacab</span>
                                @elseif ($recent->validasi_kepalacabang === 1 && $recent->validasi_admin === null)
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400">Menunggu
                                        Admin</span>
                                @elseif ($recent->validasi_admin === 1 && $recent->validasi_kepalacabang === 1)
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400">Di-ACC</span>
                                @else
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-medium bg-red-500/20 text-red-400">Ditolak</span>
                                @endif
                            </div>
                        </div>
                    @endforeach
