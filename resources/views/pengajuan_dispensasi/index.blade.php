...
@section('page-description', 'Kelola pengajuan dispensasi karyawan')
    ...
    <div class="space-y-6">
        ...
        <div class="glass-card rounded-2xl p-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-white mb-2">Daftar Pengajuan Dispensasi</h2>
                    <p class="text-gray-400">Kelola dan pantau pengajuan dispensasi karyawan</p>
                </div>
                <a href="{{ route('pengajuandispensasi.addView') }}"
        ...
        <div class="glass-card rounded-2xl p-6">
            ...
            <form method="GET" action="{{ route('pengajuandispensasi.view') }}" class="flex items-end gap-4">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-700/50">
                        @forelse($dataPengajuan as $dispensasi)
                            <tr class="hover:bg-gray-800/30 transition-colors duration-200">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            <div class="text-white font-medium">{{ $dispensasi->staff->nama }}</div>
                                            <div class="text-gray-400 text-sm">
                                                {{ $dispensasi->staff->jabatan->first()->nama_jabatan ?? 'Jabatan tidak tersedia' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-gray-300">
                                        <div class="font-medium">{{ $dispensasi->created_at->format('d M Y') }}</div>
                                        <div class="text-sm text-gray-400">{{ $dispensasi->created_at->format('H:i') }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <span
                                            class="text-purple-400 font-semibold">{{ $dispensasi->detail_pengajuan_dispensasi->count() }}
                                            Hari</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    @if (is_null($dispensasi->validasi_admin))
                                        <span
                                    ...
                                    @elseif ($dispensasi->validasi_admin == 1)
                                        <span
                                ...
                                <td class="px-6 py-4">
                                    @if ($dispensasi->admin)
                                        <div class="flex items-center">
                                            <div
                                                class="w-8 h-8 bg-gradient-to-br from-green-400/20 to-green-500/20 rounded-full flex items-center justify-center mr-2">
                                                <i class="fas fa-user-shield text-green-400 text-xs"></i>
                                            </div>
                                            <span class="text-gray-300">{{ $dispensasi->admin->nama }}</span>
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center">
                                        <a href="{{ route('pengajuandispensasi.detail', $dispensasi->id) }}"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-400 text-lg">Tidak ada pengajuan dispensasi</p>
                                        <p class="text-gray-500 text-sm">Silakan buat pengajuan baru atau ubah filter
                                            pencarian</p>
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="glass-card rounded-2xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-blue-400 font-medium">Total Hari Dispensasi</p>
                                <p class="text-lg font-bold text-white">
                                    {{ $dataPengajuan->sum(function ($item) {return $item->detail_pengajuan_dispensasi->count();}) }}
                                    Hari</p>
                            </div>
                ...
                <div class="space-y-4">
                    @foreach ($dataPengajuan->take(5) as $recent)
                        <div class="flex items-center p-4 bg-gray-800/30 rounded-lg border border-gray-700/50">
                            <div
                                class="w-10 h-10 bg-gradient-to-br from-blue-400/20 to-blue-500/20 rounded-full flex items-center justify-center mr-4">
                                <i class="fas fa-calendar-alt text-blue-400"></i>
                            </div>
                            <div class="flex-1">
                                <p class="text-white font-medium">{{ $recent->staff->nama }} mengajukan dispensasi</p>
                                <p class="text-gray-400 text-sm">{{ $recent->detail_pengajuan_dispensasi->count() }} hari
                                    • {{ $recent->created_at->diffForHumans() }}</p>
                            </div>
                            <div>
                                @if (is_null($recent->validasi_admin))
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-500/20 text-yellow-400">
                                        Menunggu
                                    </span>
                                @elseif ($recent->validasi_admin == 1)
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400">
                                        Di-ACC
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-500/20 text-red-400">
                                        Ditolak
                                    </span>
                                @endif
                            </div>
